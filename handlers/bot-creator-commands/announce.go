		...
		if server.Announce {
			...
			for _, channel := range guild.Channels {
				if channel.ID == guild.ID {
					sent = true
					s.ChannelMessageSend(channel.ID, "Admins of the server can always toggle announcements from the bot creator on/off by using `announcetoggle`. **Announcement below:**\n"+announcement)
					break
				}
			}
			...
			for _, channel := range guild.Channels {
				_, err := s.ChannelMessageSend(channel.ID, "Admins of the server can always toggle announcements from the bot creator on/off by using `announcetoggle`. **Announcement below:**\n"+announcement)
				if err == nil {
					break
				}
			}
